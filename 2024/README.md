...
struct Grid2D<Element> {
    ...
    init(repeating element: Element, rows: Int, columns: Int) {
        ...
        elements = .init(repeating: element, count: rows * columns)
        indexConversion = .init(x: 1, y: columns)
        adjacency =  [-1, -1, 0, 1, 1].uniquePermutations(ofCount: 2).map { .init(x: $0[0], y: $0[1])}
    }

    subscript(index: Index) -> Element {
        get { elements[(index &* indexConversion).wrappedSum()] }
        set { elements[(index &* indexConversion).wrappedSum()] = newValue }
    }

    subscript (_ row: Int, _ column: Int) -> Element? {
        get { self[safe: Index(row, column)] }
        set { self[safe: Index(row, column)] = newValue }
    }

    subscript(safe index: Index) -> Element? {
        get { isValid(index: index) ? self[index] : nil }
        set {
            guard isValid(index: index), let newValue else { return }
            self[index] = newValue
        }
    }

    private func isValid(index: Index) -> Bool {
        rows ~= index.y && columns ~= index.x
    }

    func depthFirstTraversal(from index: Index, matching predicate: @escaping (Element) -> Bool) -> some Sequence<Index> {
        sequence(state: (stack: [index], visited: Set<Index>())) { state in
            let found = state.stack.popThroughLast { index in
                !state.visited.contains(index) && predicate(self[index])
            }
            guard let found else { return nil }
            state.visited.insert(found)
            adjacency
                .lazy
                .map { found &+ $0 }
                .filter { [state] in
                    isValid(index: $0) && !state.visited.contains($0)
                }
                .forEach { state.stack.append($0) }
            return found
        }
    }
}
...
extension Array {
    mutating func popThroughLast(where predicate: (Element) throws -> Bool) rethrows -> Element? {
...
struct ContentView: View {
    var body: some View {
        Board(dimension: 8) {
            ForEach(0..<8) { x in
                ForEach(0..<8) { y in
                    let fillColor = (x + y).isMultiple(of: 2) ? Color.red : Color.black
                    RoundedRectangle(cornerRadius: 10)
                        .fill(fillColor)
                        .padding(2)
                        .layoutValue(key: BoardPosition.self, value: .init(x: x, y: y))
                }
            }
        }
    }
...
struct Board: Layout {
    ...
    func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout Cache) {
        let cellSize = CGSize(width: bounds.width / Double(dimension), height: bounds.height / Double(dimension))
        let transform = CGAffineTransform
            .identity
            .translatedBy(x: bounds.origin.x, y: bounds.origin.y)
            .scaledBy(x: cellSize.width, y: cellSize.height)
        for view in subviews {
            view.place(
                at: view[BoardPosition.self].applying(transform),
                proposal: .init(cellSize)
            )
        }
    }
